...
function changeEye(newVal) {
    if (stereoCam) {
        stereoCam.eyeSeparation = newVal
    }
    console.log("EYE", newVal)
}
function changeField(newVal) {
    if (stereoCam) {
        stereoCam.FOV = newVal
    }
    console.log("FIELD", newVal)
}
function changeNear(newVal) {
    if (stereoCam) {
        stereoCam.nearClippingDistance = parseFloat(newVal)
    }
    console.log("NEAR", newVal)
}
function changeConvergence(newVal) {
    if (stereoCam) {
        stereoCam.convergence = newVal
    }
    console.log("CONVERGENCE", newVal)
}
...
function draw() {
    ...
    const m = 1
    let matrOrth = m4.orthographic(-1 * m, 1 * m, -1 * m, 1 * m, -1, 1);
    ...
    if (iTextureWebCam != -1) {
        ...
        gl.uniform1i(shProgram.iUseTexture, true);
    ...
    let matAccum0 = m4.multiply(newRotMat, modelView);
    ...
    gl.uniform1i(shProgram.iUseTexture, false);
    surface.Draw();
    ...
    matAccum0 = m4.multiply(newRotMat, modelView);
...
function initGL() {
    ...
    shProgram.iSampler = gl.getUniformLocation(prog, "texture0");
    ...
    surface.BufferData(data.verticesF32, data.indicesU16, data.verticesF32);

    surfaceWebCam = new Model('SurfaceWebCam');
    // TODO: Place your code here to load two triangle geomtery
    let planeData = {};

    CreatePlaneData(planeData)
    surfaceWebCam.BufferData(planeData.verticesF32, planeData.indicesU16, planeData.uvsF32);
...
function init() {
    ...
    socket.on("data", (data) => {
        ...
        newRotMat = new Float32Array(accelerometer)
